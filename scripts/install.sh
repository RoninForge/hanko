#!/bin/sh
# Install hanko, the Claude Code plugin manifest validator.
#
# Usage:
...
#
# Environment:
#   HANKO_VERSION   pin a specific version (default: latest)
#   PREFIX          install prefix (default: /usr/local or ~/.local)
#   BIN_DIR         binary destination (default: $PREFIX/bin)
#   HANKO_NO_COLOR  set to disable colored output
#
# This script runs under dash and bash. It does not rely on `bash`-only
# features. Exit codes are non-zero on any failure.

set -eu

REPO="RoninForge/hanko"
BIN="hanko"

# --- Logging --------------------------------------------------------------

if [ -t 1 ] && [ -z "${HANKO_NO_COLOR:-}" ]; then
...
detect_os() {
  ...
  case "$uname_s" in
    Darwin) echo "darwin" ;;
    Linux)  echo "linux" ;;
    *) error "unsupported OS: $uname_s (darwin and linux are supported; windows users should download from GitHub Releases)" ;;
  esac
}
...
resolve_version() {
  if [ -n "${HANKO_VERSION:-}" ]; then
    # Allow either "v0.1.0" or "0.1.0".
    case "$HANKO_VERSION" in
      v*) echo "$HANKO_VERSION" ;;
      *)  echo "v$HANKO_VERSION" ;;
    esac
    return
  fi
  # Default: GitHub redirects /releases/latest to the actual tag URL.
  # We follow the redirect and extract the tag from the Location header.
  # Using `-o /dev/null -w '%{url_effective}'` keeps this dependency-free.
  if command -v curl >/dev/null 2>&1; then
    url=$(curl -sIL -o /dev/null -w '%{url_effective}' "https://github.com/$REPO/releases/latest")
  else
    # wget's -S emits headers; pick Location.
...
choose_bin_dir() {
  ...
  # Default: /usr/local/bin if we can write there without sudo,
  # otherwise ~/.local/bin (create it if missing).
...
info "next step: run '$BIN version' to confirm"
